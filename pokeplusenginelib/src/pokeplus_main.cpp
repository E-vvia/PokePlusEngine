...
namespace PokePlusLib {
    ...
    bool Init() {
       SDL_CHECK(SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_EVENTS), "SDL Init error");
    }
    ...
    bool Loop() {
        ...
        while (!quit) {
            ...
            const float expected_frame_duration = 1000.0f / static_cast<float>(target_fps);
            const Uint64 current_update = SDL_GetPerformanceCounter();
            const Uint64 elapsed_since_update = current_update - last_update;
            const double since_update_ms = 1000.0 * static_cast<double>(elapsed_since_update) / static_cast<double>(perf_freq);
            ...
            if (since_render_ms >= expected_frame_duration) {
                ...
                Scene* current_scene = GetActiveScene();
                if(current_scene != nullptr)
                    current_scene->Update();
                RenderPresent();
                delta_time = since_render_ms;
                since_render_ms = 0;
                fps_counter++;
            }
